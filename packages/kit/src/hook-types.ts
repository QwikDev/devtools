/**
 * Runtime devtools hook exposed on `window.__QWIK_DEVTOOLS_HOOK__`
 * by the `@devtools/plugin` in dev mode.
 *
 * Provides structured access to component state, signals, and render
 * events for both the in-app overlay and the browser extension.
 */
export interface QwikDevtoolsHook {
  ...
  /**
   * Read the current value of a signal reference.
   * Only works in-page (not serializable for extension eval).
   */
  getSignalValue(signal: unknown): unknown;

  /**
   * Return a serializable snapshot of all tracked signals grouped by
  ...
  /**
   * Return a serializable snapshot of all tracked components with
   * their hooks and current signal values.
   */
  ...
  /**
   * Subscribe to CSR render events.
   ...
   * Returns an unsubscribe function.
   *
   * Note: v1 does not implement real-time signal tracking.
   * Consumers should poll {@link getSignalsSnapshot} and diff.
   */
  onSignalUpdate(
    callback: (info: QwikDevtoolsSignalEvent) => void,
  ): () => void;
}

/** Serializable signal snapshot grouped by component path. */
export type QwikDevtoolsSignalsSnapshot = Record<
  string,
  QwikDevtoolsSignalEntry[]
>;
