...
export const DevtoolsPanel = component$((props: DevtoolsPanelProps) => {
  ...
  useVisibleTask$(({ track, cleanup }) => {
    ...
    const handleWindowResize = () => {
      if (props.state.isPanelFullscreen) return;
      updatePanelBounds(
        panelBounds,
        props.state,
        ...
        {!props.state.isPanelFullscreen && (
          <>
            ...
            <button
              ...
              class="absolute bottom-4 right-0 top-4 z-20 w-2 cursor-ew-resize"
              onMouseDown$={(event) => handleResizeStart('e', event)}
            />
            <button
              type="button"
              aria-label="Resize west"
              class="absolute bottom-4 left-0 top-4 z-20 w-2 cursor-ew-resize"
              onMouseDown$={(event) => handleResizeStart('w', event)}
            />
            <button
              type="button"
              aria-label="Resize northwest"
              class="absolute left-0 top-0 z-20 h-4 w-4 cursor-nwse-resize"
              onMouseDown$={(event) => handleResizeStart('nw', event)}
            />
            <button
              type="button"
              aria-label="Resize northeast"
              class="absolute right-0 top-0 z-20 h-4 w-4 cursor-nesw-resize"
            ...
            <button
              ...
              class="absolute bottom-0 right-0 z-20 h-4 w-4 cursor-nwse-resize"
          ...
          >
            ...
            >
              <button
                type="button"
                aria-label={
                  props.state.isPanelFullscreen ? 'Restore window' : 'Enter fullscreen'
                }
                class="bg-card-item-bg hover:bg-card-item-hover-bg border-glass-border text-muted-foreground hover:text-foreground flex h-9 w-9 items-center justify-center rounded-full border backdrop-blur-md transition-all hover:scale-105 active:scale-95 cursor-pointer shadow-sm"
