...
const inferOriginKind = (
  ...
  if (VITE_PLUGIN_INJECTED_RE.test(href)) return 'vite-plugin-injected';
  if (NODE_MODULES_RE.test(href)) return 'node_modules';
  if (VIRTUAL_MODULE_RE.test(href)) return 'virtual-module';
  if (GENERATED_MODULE_RE.test(href)) return 'generated';
...
const inferPhase = (detail: Record<string, any>, fallback: QwikPreloadPhase = 'csr') => {
  ...
  if (phase === 'ssr' || phase === 'csr' || phase === 'unknown') return phase;
  return fallback;
};
...
const syncImportDuration = (entry: QwikPreloadEntryRemembered, importDuration?: number) => {
  if (typeof importDuration !== 'number') return;
...
const syncLoadDuration = (entry: QwikPreloadEntryRemembered, loadDuration?: number) => {
  if (typeof loadDuration !== 'number') return;
...
const mergeEntry = (
  ...
  if (payload.href && !entry.href) entry.href = payload.href;
  if (payload.rel) entry.rel = payload.rel;
  if (payload.as) entry.as = payload.as;
  if (payload.resourceType && entry.resourceType === 'other') entry.resourceType = payload.resourceType;
  ...
  if (typeof payload.requestedAt === 'number') entry.requestedAt = payload.requestedAt;
  if (typeof payload.completedAt === 'number') entry.completedAt = payload.completedAt;
  if (typeof payload.importDuration === 'number') syncImportDuration(entry, payload.importDuration);
  if (typeof payload.duration === 'number' && typeof payload.importDuration !== 'number') {
    syncImportDuration(entry, payload.duration);
  }
  if (typeof payload.transferSize === 'number') entry.transferSize = payload.transferSize;
  if (typeof payload.decodedBodySize === 'number') entry.decodedBodySize = payload.decodedBodySize;
  if (payload.initiatorType) entry.initiatorType = payload.initiatorType;
  if (payload.status) entry.status = payload.status;
  if (payload.qrlSymbol) entry.qrlSymbol = payload.qrlSymbol;
  if (typeof payload.qrlRequestedAt === 'number') entry.qrlRequestedAt = payload.qrlRequestedAt;
  if (typeof payload.loadDuration === 'number') syncLoadDuration(entry, payload.loadDuration);
  ...
  if (payload.loadMatchQuality) entry.loadMatchQuality = payload.loadMatchQuality;
  if (payload.originKind && payload.originKind !== 'unknown') entry.originKind = payload.originKind;
  if (payload.phase && payload.phase !== 'unknown') entry.phase = payload.phase;
  if (payload.matchedBy && payload.matchedBy !== 'none') entry.matchedBy = payload.matchedBy;
...
const createEntry = (
  store: MutablePreloadStore,
  payload: Partial<QwikPreloadEntryRemembered> & { href: string; normalizedHref: string },
) => {
...
const upsertEntry = (
  ...
  if (activeTarget) {
    return mergeEntry(activeTarget, { ...payload, href, normalizedHref, phase });
  }
...
export function findBestQrlRequestMatch(
  ...
  if (request.normalizedHref) {
    for (let i = entries.length - 1; i >= 0; i--) {
      ...
      if (entry.qrlRequestedAt != null && entry.qrlSymbol !== request.symbol) continue;
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    ...
    if (!request.symbol || entry.qrlSymbol !== request.symbol) continue;
    return {
      entryId: entry.id,
  ...
  for (let i = entries.length - 1; i >= 0; i--) {
    const entry = entries[i];
    if (entry.phase !== phase || !isScriptLikeEntry(entry)) continue;
    if (entry.qrlRequestedAt != null && entry.qrlSymbol !== request.symbol) continue;
    ...
    if (entryCompletedAt + 50 < request.requestedAt) continue;
    if (distance > QRL_MATCH_WINDOW_MS || distance >= bestDistance) continue;
...
const tryMatchRequest = (
  ...
  if (typeof match.entryId !== 'number') return undefined;
...
const tryMatchPendingRequestsForEntry = (
  ...
  for (let i = store.qrlRequests.length - 1; i >= 0; i--) {
    ...
    if (request.matchedEntryId) continue;

    const match = findBestQrlRequestMatch([entry], request);
    if (match.entryId !== entry.id) continue;
...
const finalizeEntry = (
  ...
  entry.status = status;
  if (typeof entry.completedAt !== 'number') entry.completedAt = completedAt;
...
const recordLink = (
  ...
  if (!['preload', 'modulepreload', 'prefetch'].includes(rel)) return;
...
const applyResourceTiming = (store: MutablePreloadStore, resourceEntry: PerformanceEntry) => {
  if (!('name' in resourceEntry)) return;
  ...
  if (!entry && shouldCreateFromResourceTiming(timing)) {
    entry = createEntry(store, {
      ...
      resourceType: inferResourceType(undefined, timing.name || '', timing.initiatorType || undefined),
      source: 'performance',
      status: 'loaded',
...
const ingestSsrPreloads = (
  ...
  for (const raw of rawEntries) {
    ...
    const normalizedHref = typeof raw.normalizedHref === 'string'
      ? raw.normalizedHref
      : normalizeHref(href);
    ...
    const entry = upsertEntry(
      store,
      {
        ...
        decodedBodySize:
          typeof raw.decodedBodySize === 'number' ? raw.decodedBodySize : undefined,
        ...
        phase: 'ssr',
        matchedBy:
          typeof raw.matchedBy === 'string' ? raw.matchedBy : 'none',
...
export function ensurePreloadRuntime() {
  ...
  window.addEventListener('error', (event) => {
    const href = extractHrefFromUnknownError(event.error) || extractHrefFromUnknownError(event.message);
    if (!href) return;
    recordWindowError(
      store,
      href,
      'error',
      typeof event.message === 'string' ? event.message : 'Failed to fetch dynamically imported module',
    );
  });
