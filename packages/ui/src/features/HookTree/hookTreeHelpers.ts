...
export function toTreeNodes(vnodes: VNodeTreeNode[]): TreeNode[] {
  return vnodes.map((v) => ({
    ...
    name: v.name,
    label: v.name,
    props: v.props as Record<string, string | number | boolean | null | undefined | object> | undefined,
...
export function findNodeByDomAttr(nodes: TreeNode[], qId: string | null, colonId: string | null): TreeNode | null {
  for (const node of nodes) {
    const props = node.props as Record<string, unknown> | undefined;
    if (qId && props?.['q:id'] === qId) return node;
    if (colonId && props?.__colonId === colonId) return node;
    if (node.children) {
...
export function valueToTree(key: string, val: unknown, depth: number): TreeNode | null {
  ...
  if (isSerializedFunction(val)) {
    ...
    return { id: nextId(), label: `${key}: fn ${name}()`, elementType: 'function' };
  }

  const t = typeof val;

  if (t === 'string' || t === 'number' || t === 'boolean') {
    return { id: nextId(), name: key, label: `${key}: ${JSON.stringify(val)}`, elementType: getElementType(val) };
  }
  ...
  if (Array.isArray(val)) {
    ...
    return { id: nextId(), name: key, label: `${key} (${val.length})`, elementType: 'array', children: children.length > 0 ? children : undefined };
  }
  ...
  if (t === 'object') {
    ...
    return { id: nextId(), name: key, label: displayLabel, elementType: 'object', children: children.length > 0 ? children : undefined };
  }
