...
function valueToTree(key: string, val: unknown, depth: number): TreeNode | null {
  ...
  if (t === 'boolean' || t === 'number') {
    return { id: nextId(), label: `${key}: ${val}`, elementType: getElementType(val) };
  }
  ...
  if (t === 'object' && val && (val as Record<string, unknown>).__type === 'function') {
    ...
    return { id: nextId(), label: `${key}: ƒ ${name}()`, elementType: 'function' };
  }
  ...
  if (t === 'object' && val) {
    ...
    const label = className && className !== 'Object'
      ? `${key}: Class {${className}}`
  ...
  return { id: nextId(), label: `${key}: ${String(val)}`, elementType: 'string' };
}
  ...
  for (const [hookType, hooks] of groups) {
    ...
    for (const hook of hooks) {
      ...
      if (data && typeof data === 'object' && !Array.isArray(data)) {
        ...
        const sizeLabel = className && className !== 'Object'
          ? `Class {${className}}`
        ...
        const formatted = data === null ? 'null'
  ...
  requestAnimationFrame(() => {
    ...
    while (parent && parent.id !== 'hook-tree-left') {
      ...
      if (parent.style.maxHeight === '0px') {
        ...
        if (siblingRow) (siblingRow as HTMLElement).click();
      }
...
export const HookTree = component$(() => {
  ...
  const handleElementPicked = $((payload: { qId?: string; colonId?: string; treeNodeId?: string }) => {
  ...
  const refreshSelectedDetail = $(async () => {
    ...
    if (vnodeProps && Object.keys(vnodeProps).length > 0) {
      entries.push({ hookType: 'props', variableName: 'props', data: vnodeProps });
    }

    if (entries.length > 0) {
      const full = buildDetailTree(entries);
      fullStateTree.value = full;
      const existingMap = new Map<string, boolean>(hookFilters.value.map((f) => [f.key, f.display]));
  ...
  const onNodeClick = $(async (node: TreeNode) => {
    ...
    const qrlChunk = (node.props as Record<string, unknown> | undefined)?.__qrlChunk as string | undefined;
  ...
  const onStateNodeClick = $(async (node: TreeNode) => {
    ...
    editValue.value = rawVal.startsWith('"') && rawVal.endsWith('"')
      ? rawVal.slice(1, -1)
      : rawVal;
  });
  ...
  const handleClear = $(async () => {
    hookFilters.value = hookFilters.value.map((f) => ({ ...f, display: false }));
    stateTree.value = await applyFilters(fullStateTree.value, hookFilters.value);
  });
      ...
      <div class="flex h-full w-full">
        ...
        <div class="flex h-full min-h-0 w-1/2 flex-col overflow-hidden p-4">
                      ...
                      if (editingNodeId.value === node.id) {
                        return (
                          ...
                          >
                            <input
                              type="text"
                              class="w-32 rounded border border-primary/50 bg-card-item-hover-bg px-1.5 py-0.5 text-xs text-foreground outline-none focus:border-primary"
                            ...
                            <button
                              type="submit"
                              class="rounded bg-primary/20 px-1.5 py-0.5 text-xs text-primary hover:bg-primary/30"
                            >
                              ok
                            </button>
                          </form>
          ...
          {currentTab.value === 'code' && (
            <div class="mt-4 flex min-h-0 flex-1 items-center justify-center rounded-xl border border-glass-border bg-card-item-bg p-4">
              <div class="text-center">
                ...
                <div class="text-muted-foreground/60 mt-2 text-xs">
                  ...
                  <code class="rounded bg-card-item-hover-bg px-1.5 py-0.5">
                    pnpm dev
                  </code>{' '}
                  with{' '}
                  <code class="rounded bg-card-item-hover-bg px-1.5 py-0.5">
