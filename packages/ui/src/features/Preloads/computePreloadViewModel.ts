...
function averageBy(entries: QwikPreloadEntryRemembered[], field: 'importDuration' | 'loadDuration'): number {
...
function matchesOrigin(row: PreloadRowVm, filters: PreloadSourceFilters): boolean {
  if (row.originKind === 'current-project') return true;
...
function matchesPhase(row: PreloadRowVm, filters: PreloadViewFilters['phaseFilters']): boolean {
  if (row.phase === 'unknown') return false;
...
export function computePreloadViewModel(
  ...
  const visibleEntries = entries.filter((entry) => {
    ...
    if (origin === 'current-project') return true;
    ...
    if (entry.phase === 'unknown') return false;
