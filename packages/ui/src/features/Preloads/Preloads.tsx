...
export const Preloads = component$(() => {
  ...
  const sourceFilters = useStore<PreloadSourceFilters>({ ...DEFAULT_FILTERS.sourceFilters });
  ...
  return (
    <div class="h-full w-full flex-1 overflow-hidden">
      <div class="flex h-full min-h-0 flex-col gap-4">
        ...
        <div class="border-glass-border flex min-h-0 flex-1 flex-col overflow-hidden rounded-2xl border bg-card-item-bg">
          <div class="border-glass-border flex flex-wrap items-start justify-between gap-4 border-b p-4">
            <div class="flex flex-col gap-3">
              ...
              <div class="flex flex-wrap gap-2">
                <div class="text-muted-foreground mr-1 text-xs font-medium uppercase tracking-wide">
                  Sources
                </div>
                ...
                <label class="border-border flex cursor-pointer items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium">
                  ...
                  />
                  {makeLabelCount('Vite Plugin Injected', vm.value.sourceCounts['vite-plugin-injected'])}
                </label>
                <label class="border-border flex cursor-pointer items-center gap-2 rounded-full border px-3 py-1.5 text-xs font-medium">
                  <input
                    type="checkbox"
          ...
          <div class="min-h-0 flex-1 overflow-auto">
            <table class="w-full min-w-[1080px] text-left text-sm">
              <thead class="bg-background/60 text-muted-foreground sticky top-0 z-10 text-xs uppercase tracking-wide">
                <tr>
                ...
                {rows.value.length ? (
                  rows.value.map((row) => (
                    <tr key={row.id} class="border-glass-border border-t align-top">
                      ...
                      <td class="px-4 py-3">
                        ...
                        <div class="text-muted-foreground mt-1 text-[11px] uppercase tracking-wide">
                          {row.originKind}
                        </div>
                      </td>
                      <td class="px-4 py-3">
                        <div class="font-medium uppercase tracking-wide">{row.phase}</div>
                      </td>
                      <td class="px-4 py-3">
                  ...
                  <tr class="border-glass-border border-t">
                    <td class="text-muted-foreground px-4 py-8 text-center text-sm" colSpan={7}>
                      No rows match the current filters. Uncheck a filter or re-enable CSR to view data.
                    </td>
                  </tr>
                )}
              </tbody>
