...
/**
 * QRLDev contains development metadata.
 * Exists in @qwik.dev/core but not exported as a type.
 *
...
/**
 * Internal QRL properties that exist at runtime.
 * These are part of `QRLInternalMethods` in @qwik.dev/core but NOT exported.
 *
...
/**
 * QRL with internal properties accessible.
 * Combines public QRL type with internal runtime properties.
 *
 * Note: `QRLInternal<T>` exists in Qwik but is NOT exported.
 * We define our own to access `$chunk$` and `$captureRef$`.
 */
...
export function isQRLInternal(qrl: unknown): qrl is QRLInternal {
  if (typeof qrl !== 'object' || qrl === null) return false;
  return '$chunk$' in qrl || '$symbol$' in qrl;
}

/**
 * Safely cast QRL to QRLInternal for accessing internal properties.
 * Use this instead of `as unknown as Record<string, unknown>`.
 */
