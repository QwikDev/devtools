import type { QwikPerfEntryRemembered, QwikPerfStoreRemembered } from '@devtools/kit';
...
export type PerfSsrItem = QwikPerfEntryRemembered & { phase: 'ssr'; ssrCount?: number };
...
export function groupCsrBySsr(data: PerfPayload): WeakMap<PerfSsrItem, PerfGroupedCsrItem[]> {
  ...
  for (const ssrItem of data.ssr) {
    ...
    ssrByName.set(parseComponentAndEventName(ssrItem.component).componentName, ssrItem as PerfSsrItem);
  }
