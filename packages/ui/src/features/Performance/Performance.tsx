...
export const Performance = component$(() => {
  ...
  useVisibleTask$(async ({ cleanup }) => {
    ...
    if (all.length) {
      renderEvents.value = all.map((e: any) => ({
  ...
  return (
    <div class="h-full w-full flex-1 overflow-y-auto">
      <div class="flex min-h-0 flex-col gap-4">
        ...
        {hasData && (
          <div class="border-glass-border flex min-h-0 overflow-hidden rounded-2xl border bg-card-item-bg">
        ...
        {/* Live Render Events */}
        <div class="border-glass-border rounded-2xl border bg-card-item-bg p-4">
          <div class="mb-3 flex items-center justify-between">
            <div>
          ...
          {!hasRenderEvents ? (
            ...
            <div class="max-h-[400px] overflow-y-auto">
              <table class="w-full text-sm">
                <thead>
                  <tr class="text-muted-foreground border-b border-glass-border text-left text-xs">
                    <th class="py-1.5 pr-3 font-medium">Time</th>
                    <th class="py-1.5 pr-3 font-medium">Component</th>
                    <th class="py-1.5 pr-3 font-medium text-right">Duration</th>
                    <th class="py-1.5 font-medium">Phase</th>
                  </tr>
                </thead>
                <tbody>
                  {renderEvents.value.slice().reverse().map((evt, i) => (
                    <tr key={i} class="border-b border-glass-border/50 last:border-0">
