...
export const AsyncStatePanel = component$(() => {
  ...
  const activePreset = useComputed$(
    () =>
      experimentPresets.find((preset) => preset.id === lab.presetId.value) ??
      experimentPresets[1],
  );
  ...
  const snapshot = useResource$<SyntheticPayload>(async ({ track }) => {
    ...
    const preset =
      experimentPresets.find((item) => item.id === presetId) ??
      experimentPresets[1];
      ...
      <Resource
          ...
          <div class="resource-card resource-card--pending">
            ...
            <div class="resource-copy">
              ...
              <div class="resource-body">
                Holding for {activePreset.value.latencyMs}ms to make the pending
                state inspectable in devtools.
              </div>
            </div>
          </div>
        )}
        onRejected={(error) => (
          <div class="resource-card resource-card--error">
            ...
            <div class="resource-body">
              The UI footprint stays stable so that failure remains visually
              comparable to resolved states.
            </div>
          </div>
        )}
        onResolved={(payload) => (
