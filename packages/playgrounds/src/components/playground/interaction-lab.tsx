...
export const InteractionLab = component$<InteractionLabProps>(({ serverTime }) => {
  ...
  const activePreset = useComputed$(
    () =>
      experimentPresets.find((preset) => preset.id === presetId.value) ??
      experimentPresets[1],
  );

  const activityIndex = useComputed$(() => {
    const intensityBoost =
      surfaceIntensity.value === 'surge'
        ? 22
  ...
  const applyLayout = $((nextLayout: LayoutMode) => {
    ...
    eventLog.value = [
      createLabEvent(
        'Layout updated',
        `${nextLayout} mode rearranged the experiment surface.`,
      ),
      ...eventLog.value,
    ].slice(0, 6);
  });
    ...
    >
      <div class="lab-panel lab-panel--controls">
        ...
        <div class="quick-actions">
          <button class="action-button action-button--primary" onClick$={triggerAsync} type="button">
            Trigger async load
          </button>
