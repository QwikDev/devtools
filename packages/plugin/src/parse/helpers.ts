...
/**
 * Checks if a hook name follows the custom hook naming convention (use[A-Z_])
 * and is not a built-in Qwik hook
 */
...
export function hasCollecthookAfterByVariableName(
  ...
  const lookahead = code.slice(fromIndex, fromIndex + maxLookahead);
  const pattern = new RegExp(`collecthook\\s*\\(\\s*\\{[\\s\\S]{0,200}?variableName:\\s*'${variableName}'`);
  return pattern.test(lookahead);
}
