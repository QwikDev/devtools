/**
 * Virtual module that bridges Qwik VNode internals to the devtools hook.
 *
 * Imported as a side-effect from root.tsx by the devtools plugin.
 * Since it does NOT use Qwik's `$()` functions, the Qwik compiler
 * leaves it untouched (no QRL extraction).
 *
 * Exposes `window.__QWIK_DEVTOOLS_HOOK__.getVNodeTree()` which returns
 * a serializable component tree built from the live VNode structure.
 */
const vnodeBridge = `
import {
  _getDomContainer,
...
function serializeProps(val, depth) {
  ...
  try {
    if (Array.isArray(val)) {
      return val.map(function(item) { return serializeProps(item, depth + 1); });
    }
    if (t === 'object') {
      // Skip Qwik internal objects (QRLs, signals)
      ...
      for (var i = 0; i < keys.length; i++) {
        ...
        try { result[key] = serializeProps(val[key], depth + 1); } catch(_) { result[key] = '[error]'; }
      }
      return result;
    }
  } catch(_) {}
...
function buildTree(container, vnode) {
  ...
  while (current) {
    ...
    var renderFn = isVirtual
      ? container.getHostProp(current, QRENDERFN)
      : null;
    ...
    if (isComponent) {
      ...
      try {
        ...
        for (var i = 0; i < keys.length; i++) {
          ...
          if (keys[i] === 'q:id') {
            qId = String(container.getHostProp(current, 'q:id') || '');
          }
          if (keys[i] === ':') {
            colonId = String(container.getHostProp(current, ':') || '');
          }
        }
        if (renderFn.getSymbol) {
          name = normalizeName(renderFn.getSymbol());
        } else if (renderFn.$symbol$) {
      ...
      _vnode_isMaterialized(current) ||
      (isVirtual && !isComponent)
    ) {
...
function setupBridge() {
  ...
  // Resolve a DOM element to the nearest component tree node ID.
  // Walks up from the element checking data-qwik-inspector to match component names.
  ...
  // Walk from a VNode down to the first real DOM Element.
  // _ElementVNode extends _VirtualVNode and has a .node (Element) property.
  function findDomElement(vnode) {
    if (!vnode) return null;
    // ElementVNode has a .node property (the actual DOM element)
    if (!_vnode_isVirtualVNode(vnode) || vnode.node) {
      return vnode.node || null;
    }
    // Virtual-only: walk first child
  ...
  hook.highlightNode = function(nodeId, name) {
    ...
    try {
      ...
      if (!ov) {
        ...
        ov.style.cssText = 'position:fixed;pointer-events:none;border:2px solid #8b5cf6;background:rgba(139,92,246,0.08);z-index:2147483646;border-radius:4px;transition:all 0.15s ease';
        var lbl = document.createElement('div');
        lbl.id = '__qwik_dt_hover_lbl';
        lbl.style.cssText = 'position:absolute;top:-20px;left:-2px;background:#8b5cf6;color:#fff;font-size:10px;padding:1px 6px;border-radius:3px 3px 0 0;white-space:nowrap;font-family:system-ui,sans-serif';
        ov.appendChild(lbl);
  ...
  hook.getNodeProps = function(nodeId) {
    ...
    try {
      ...
      for (var i = 0; i < keys.length; i++) {
        var key = keys[i];
        // Skip event handlers (on:click etc)
        if (key.startsWith('on:') || key.startsWith('on$:')) continue;
        try { result[key] = serializeProps(props[key], 0); } catch(_) { result[key] = '[error]'; }
      }
      return result;
    } catch(_) {
      return null;
...
function filterDevtools(nodes) {
  ...
  for (var i = 0; i < nodes.length; i++) {
    ...
    if (n.children) {
      n = { name: n.name, id: n.id, label: n.label, props: n.props, children: filterDevtools(n.children) };
...
export const VNODE_BRIDGE_KEY = 'virtual:qwik-devtools-bridge';
