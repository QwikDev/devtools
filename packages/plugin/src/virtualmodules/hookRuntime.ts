 ...
 *
 * This file exports a **string** (not executable TS) that gets appended
 * to {@link perfRuntime}. Since perfRuntime is concatenated into plain JS
 * modules (`virtual:qwik-component-proxy`, lazy render wrappers), the
 * hook initialises before any Qwik component renders.
 *
 * Signal values are read directly from `QWIK_DEVTOOLS_GLOBAL_STATE`:
 * each hook entry has `data` set to the actual signal/store reference
 * by the `collecthook()` instrumentation. The hook reads `.value`
 * from those references at snapshot time.
 */
const hookRuntime = `
