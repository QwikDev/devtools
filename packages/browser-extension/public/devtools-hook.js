 ...
 * Sets up window.__QWIK_DEVTOOLS_HOOK__ with signal tracking, component snapshots,
 * and state editing. Skips if the Vite plugin already installed the hook.
 *
 * NOTE: This duplicates logic from plugin/virtualmodules/hookRuntime.ts.
 * Both must stay in sync. The duplication is intentional: the plugin injects
 * via SSR middleware, while the extension injects via content script.
 *
  ...
  function deepSerialize(val, depth) {
    ...
    try {
      ...
      if (Array.isArray(val)) {
        return val.map(function (item) { return deepSerialize(item, depth + 1); });
      }
      if (t === 'object') {
        ...
        for (var i = 0; i < keys.length; i++) {
          ...
          try { result[key] = deepSerialize(val[key], depth + 1); } catch (_) { result[key] = '[unreadable]'; }
        }
        return result;
      }
    } catch (_) {}
  ...
  window.__QWIK_DEVTOOLS_HOOK__ = {
    ...
    _emitRender: function (info) {
      for (var i = 0; i < renderListeners.length; i++) {
        try { renderListeners[i](info); } catch (_) {}
      }
    },
      ...
      for (var p = 0; p < paths.length; p++) {
        ...
        for (var h = 0; h < hooks.length; h++) {
          if (signalTypes[hooks[h].hookType] && hooks[h].data != null) {
            signals.push({ name: hooks[h].variableName || '', hookType: hooks[h].hookType, value: readValue(hooks[h].data) });
          }
        }
    ...
    getComponentTreeSnapshot: function () {
      ...
      return Object.keys(state).map(function (path) {
        ...
        for (var i = 0; i < hooks.length; i++) {
          hookEntries.push({ variableName: hooks[i].variableName || '', hookType: hooks[i].hookType || '', category: hooks[i].category || '' });
          if (signalTypes[hooks[i].hookType] && hooks[i].data != null) {
            signals.push({ name: hooks[i].variableName || '', hookType: hooks[i].hookType, value: readValue(hooks[i].data) });
          }
        }
    ...
    getComponentDetail: function (componentName, qrlChunk) {
      ...
      for (var i = 0; i < comp.hooks.length; i++) {
        var h = comp.hooks[i];
        if (h.data != null) {
          result.push({ hookType: h.hookType || 'unknown', variableName: h.variableName || h.hookType || 'unknown', data: deepSerialize(h.data, 0) });
        }
      }
      return result;
    },
    ...
    setSignalValue: function (componentName, qrlChunk, variableName, newValue) {
      ...
      for (var i = 0; i < comp.hooks.length; i++) {
        ...
        if (h.variableName === variableName && h.data != null) {
          try {
            if (typeof h.data === 'object' && 'value' in h.data) { h.data.value = newValue; return true; }
          } catch (_) {}
        }
      }
      return false;
    ...
    onSignalUpdate: function () { return function () {}; },
  };
})();
