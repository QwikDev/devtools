/**
 * VNode bridge - injected by the browser extension as an ES module.
 * Bridges Qwik VNode internals to the devtools hook.
 *
 * NOTE: This duplicates logic from plugin/virtualmodules/vnodeBridge.ts
 * and the EVAL_INSTALL_BRIDGE in extension-data-provider.ts. All three
 * must stay in sync. The duplication exists because each runs in a
 * different context (Vite SSR, ES module, inspectedWindow.eval).
 *
 * Requires @qwik.dev/core/internal to be resolvable (works in dev mode
 * where Vite serves bare module imports).
 *
 * Skips silently if the Vite plugin already set up the bridge
 * (checks hook.getVNodeTree existence).
 */
import {
  _getDomContainer,
...
function buildTree(container, vnode) {
  ...
  while (current) {
    ...
    if (isComponent) {
      ...
      result.push({ name: name, id: nodeId, label: name, props: nodeProps, children: children.length > 0 ? children : undefined });
