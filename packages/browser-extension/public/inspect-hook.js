 ...
 *
 * Injected into the inspected page's main world by the content script.
 * Must run in the SAME world as qwikloader so that
 * `stopImmediatePropagation` actually blocks Qwik's event handlers.
 *
 * Lifecycle:
 *   1. Content script posts `__QWIK_DT_INSPECT_START` to activate.
 *   2. User clicks an element on the page.
 *   3. This script intercepts the click (window capture, before qwikloader),
 *      resolves the nearest Qwik component, and posts
 *      `__QWIK_DT_ELEMENT_PICKED` back to the content script.
 *   4. Content script forwards the result to the devtools panel.
 *
 * Why window capture?
 *   Qwik's qwikloader registers its click handler on `document` in capture.
 *   Capture order is: window > document > html > body > ... > target.
 *   By listening on `window` we fire BEFORE qwikloader.
 */
(function () {
  ...
  /**
   * Try to resolve a DOM element to its owning Qwik component's tree node ID
   * using the devtools hook installed by the Vite plugin.
  ...
  window.addEventListener(
    ...
    function (e) {
      ...
      /** @type {{ type: string, qId: string|null, colonId: string|null, treeNodeId: string|null }} */
      const msg = {
