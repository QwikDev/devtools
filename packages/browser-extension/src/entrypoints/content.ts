...
export default defineContentScript({
  ...
  main() {
    ...
    /**
     * Walk up the DOM to find the nearest Qwik-managed ancestor.
     * Handles both v1 (`q:id`) and v2 (`:=` binding attribute).
     */
    function findQwikAncestor(el: Element): Element | null {
      let current: Element | null = el;
      while (current) {
        if (current.hasAttribute(QWIK_ATTR.ID)) return current;
        if (current.hasAttribute(V2_BINDING_ATTR)) return current;
    ...
    /**
     * Extract the Qwik component name from `data-qwik-inspector` attribute.
     * Format: "/src/components/Button/Button.tsx:49:10"
     */
    function getComponentName(el: Element): string | null {
      const inspector = el.getAttribute('data-qwik-inspector')
    ...
    function handleInspectMove(e: MouseEvent) {
      ...
      if (qwikEl) {
        ...
        const label = inspectOverlay.querySelector('#__qwik_devtools_label') as HTMLDivElement | null;
        if (label) {
    ...
    const checkUrlChange = () => {
      if (location.href !== lastUrl) {
        ...
        const container =
          document.querySelector(QWIK_CONTAINER_SELECTOR) ?? document.body;
    ...
    (
      (window as Record<string, unknown>).__qwik_dt_abort as
        | AbortController
        | undefined
    )?.abort();
    const navAbort = new AbortController();
    (window as Record<string, unknown>).__qwik_dt_abort = navAbort;
